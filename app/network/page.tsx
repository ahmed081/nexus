...
const intentColors: Record<string, string> = {
  Hiring: "bg-accent text-accent-foreground",
  Mentorship: "bg-primary text-primary-foreground",
  Collaboration: "bg-success text-success-foreground",
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {filtered.map((person) => (
          <Card key={person.name} className="transition-shadow hover:shadow-md">
            <CardContent className="p-5">
              <div className="flex flex-col items-center text-center">
                <div className="flex h-14 w-14 items-center justify-center rounded-full bg-primary text-lg font-bold text-primary-foreground">
                  {person.initials}
                </div>
                <h3 className="mt-3 font-semibold text-foreground">{person.name}</h3>
                <p className="text-sm text-muted-foreground">{person.role}</p>
                <p className="text-xs text-muted-foreground">{person.company}</p>

                {/* Intent Badge */}
                <span className={`mt-2 inline-block rounded-full px-2.5 py-0.5 text-xs font-medium ${intentColors[person.intent]}`}>
                  {person.intent}
                </span>
                ...
                <Button className="mt-3 w-full gap-1.5 bg-primary text-primary-foreground hover:bg-primary/90" size="sm">
